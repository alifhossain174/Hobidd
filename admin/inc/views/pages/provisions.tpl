        ...
        {foreach from=$data key=key item=val}
			<tr>
				...
				<td nowrap="nowrap" style="text-align:center;">
					<a href="?s=editProvision&id={$val.id}" title="Bearbeiten" class="glyphicon glyphicon-pencil"></a>
					<a href="?s=deleteProvision&id={$val.id}" id="d{$val.id}" title="Löschen"
...
{if $B_createProvision || $B_editProvision}
	<form class="form-horizontal{if $ajax} ajaxForm{/if}" role="form" method="post" action="?s={$state}&id={$data.id}"
		...
		<div id="form-header">
			<h4>Verpflegung {if $state == "createProvision"}hinzufügen{else}bearbeiten{/if}</h4>
		</div>
